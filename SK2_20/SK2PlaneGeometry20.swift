...
fileprivate func debug(_ mtd: String, _ msg: String = "") {
    if (debugEnabled) {
        ...
        os_log("SK2PlaneGeometry20.%s [%s] %s", mtd, threadName, msg)
    }
}
...
class SK2PlaneGeometry20: SK2Geometry20 {
    ...
    func connectGestures(_ view: UIView) {
        ...
        let newPan = UIPanGestureRecognizer(target: self, action: #selector(SK2PlaneGeometry20.doPan))
        self.pan = newPan
        view.addGestureRecognizer(newPan)
        
        let newPinch = UIPinchGestureRecognizer(target: self, action: #selector(SK2PlaneGeometry20.doPinch))
        self.pinch = newPinch
        view.addGestureRecognizer(newPinch)
        
        let newTap = UITapGestureRecognizer(target: self, action: #selector(SK2PlaneGeometry20.doTap))
        self.tap = newTap
