...
fileprivate func debug(_ mtd: String, _ msg: String = "") {
    if (debugEnabled)  {
        print("SK2_PrimaryViewController20", mtd, msg)
    }
}
...
class SK2_PrimaryViewController20: UIViewController, UITextFieldDelegate, AppModelUser21 {
    ...
    func visualization_setup() {
        ...
        let figureSelector = visualization.figures
    ...
    @IBAction func calibrate(_ sender: Any) {
        debug("calibrate")
        let figure = visualization.figures.selection?.value as? Calibrated
        figure?.calibrate()
    }
    
    @IBAction func resetPOV(_ sender: Any) {
        debug("resetPOV")
        visualization.figures.selection?.value.resetPOV()
    }
