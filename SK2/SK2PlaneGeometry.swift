...
fileprivate func debug(_ mtd: String, _ msg: String = "") {
    if (debugEnabled) {
        ...
        os_log("SK2PlaneGeometry.%s [%s] %s", mtd, threadName, msg)
    }
}
...
class SK2PlaneGeometry: SK2Geometry {
    ...
    func connectGestures(_ view: UIView) {
        ...
        let newPan = UIPanGestureRecognizer(target: self, action: #selector(SK2PlaneGeometry.doPan))
        self.pan = newPan
        view.addGestureRecognizer(newPan)
        
        let newPinch = UIPinchGestureRecognizer(target: self, action: #selector(SK2PlaneGeometry.doPinch))
        self.pinch = newPinch
        view.addGestureRecognizer(newPinch)
        
        let newTap = UITapGestureRecognizer(target: self, action: #selector(SK2PlaneGeometry.doTap))
        self.tap = newTap
